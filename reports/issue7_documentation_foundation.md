...
- `python3 -m unittest discover -s scripts/generated_data/tests -v` -> 633
  tests, `OK`; `generated-data-check` (removed target) -> 13 tables / 722 records, no
...
| Document generated data schemas and authoring workflows | `docs/generated_data.md` (schema/validation/generation design reference, issue #5) and `docs/generated_data_tutorial.md` (contributor authoring walkthrough); JSON sources under `src/data/*.json` + `scripts/generated_data/*/schema.py`; `generated-data-validate` (removed target)/`generated-data-generate`/`generated-data-check` -- **historical**: this JSON/generator platform was later removed in favor of hand-authored `src/data_*.c` tables | Candidate current-scope (superseded) |
