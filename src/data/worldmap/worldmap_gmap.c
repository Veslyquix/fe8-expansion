#include "global.h"

#if FE8_CUSTOM_CAMPAIGN
u8 gWorldmapGmap_0[] = INCBIN_U8("graphics/misc/WorldMapAshor.4bpp");
#else
u8 gWorldmapGmap_0[] = INCBIN_U8("graphics/misc/gUnknown_08A83364.4bpp");
#endif

u16 gWorldmapGmap_1[] = INCBIN_U16("graphics/misc/gUnknown_08A95F64.4bpp");

u16 gWorldmapGmap_2[] = INCBIN_U16("graphics/misc/gUnknown_08A95FE4.4bpp");

u16 gWorldmapGmap_3[] = INCBIN_U16("graphics/misc/gUnknown_08A96064.4bpp.lz");

u8 Img_GmapNodes[] = INCBIN_U8("graphics/misc/Img_GmapNodes.4bpp.lz");

u8 Img_GmapCastleNodes[] = INCBIN_U8("graphics/misc/Img_GmapCastleNodes.4bpp.lz");

u16 gWorldmapGmap_4[] = INCBIN_U16("graphics/misc/gUnknown_08A97A40.4bpp");

u16 gPal_GMapPI_ShopIcons[] = INCBIN_U16("graphics/misc/gPal_GMapPI_ShopIcons.gbapal");

u8 gGfx_GMapPI_ShopIcons[] = INCBIN_U8("graphics/misc/gGfx_GMapPI_ShopIcons.4bpp.lz");

#if FE8_CUSTOM_CAMPAIGN
u16 gPal_WorldmapGmap_0[] = INCBIN_U16("graphics/misc/WorldMapAshor.gbapal");
#else
u16 gPal_WorldmapGmap_0[] = INCBIN_U16("graphics/misc/gPal_08A97ACC.gbapal");
#endif
