...
CONST_DATA struct SpellAssoc gSpellAssocData[] = {
    ...
    // count=1 (not the usual _DEFAULT's 2): this dummy slot is now
    // ITEM_CALL, the internal weapon UnitCall_SetUpSoloAttackAnim
    // (src/unitcall.c, FE8_SKILLSYSTEM) passes into gBattleActor. count=1
    // is what routes the Ekr battle intro to a solo/single-unit-centered
    // scene (see ITEM_STAFF_LATONA below for the same mechanism).
...
#if FE8_CUSTOM_CAMPAIGN
    // Nostal reuses Physic's animation as-is (see data_items.c/CREDITS.md).
    SPELL_ASSOC_DATA(ITEM_STAFF_NOSTAL, 2, 0x29, ProcScr_SpellAssocPhysic, true, MA_FACING_OPPONENT, SPELL_ASSOC_MCOLOR_NORMAL),
#endif
