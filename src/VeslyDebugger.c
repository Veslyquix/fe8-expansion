...
#ifdef MODERN
#define MU_Exists MuExists
#define MU_Create StartMu
#define MU_SetDefaultFacing_Auto SetAutoMuDefaultFacing
#define MU_CreateForUI StartUiMu
#define MU_SetFacing SetMuFacing
#define MU_EndAll EndAllMus
#define gProcScr_CamMove ProcScr_CamMove
#define MapAnimProc_DisplayExpBar MapAnim_DisplayExpBar
#define sub_809F5F4 PrepItemList_SnapCursorIntoView
#define unk86 cmd_result
#define ai1data ai_a_pc
#define ai2data ai_b_pc
#define ai3And4 ai_config
...
int VeslyDebugger_TryApplyBootMode(ProcPtr aproc)
{
...
void StartPlayerPhaseTerrainWindow(void);
...
void FindNearestTile(struct Unit * unit)
{
    ...
    unit->xPos = gActiveUnit ? gActiveUnit->xPos : gBmSt.playerCursor.x;
    unit->yPos = gActiveUnit ? gActiveUnit->yPos : gBmSt.playerCursor.y;
...
{
    DebuggerProc * proc = Proc_Find(DebuggerProcCmdIdler);

    if (!proc || !proc->godMode)
        return;
...
void SetupUnitFunc(void)
{
    ...
    gBattleActor.weaponBefore = gBattleTarget.weaponBefore = itemSlot >= 0 ? unit->items[itemSlot] : 0;
...
int PageMenuItemDraw(struct MenuProc * menu, struct MenuItemProc * menuItem)
{
    ...
    DebuggerProc * procIdler = Proc_Find(DebuggerProcCmdIdler);
    char pageText[] = " Page 0/0";
    pageText[6] = '1' + procIdler->page;
    pageText[8] = '0' + NumberOfPages;
    Text_DrawString(&menuItem->text, pageText);
    PutText(&menuItem->text, BG_GetMapBuffer(menu->frontBg) + TILEMAP_INDEX(menuItem->xTile, menuItem->yTile));
    return 0;
}
...
void InitPlayerPhaseTerrainWindow(void);
...
// --- end Class Sprites battle animation preview ---
