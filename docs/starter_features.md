...
`EXPANSION_SAVE_COMPAT_EPOCH` stays at the current default `2` (introduced by issue #18), the `ExpansionSaveMeta`/save-block
...
* No raw numeric content IDs, no hand-edited generated C, no second
  ...
  starter-driven save-epoch bump (`EXPANSION_SAVE_COMPAT_EPOCH` remains the current default `2`).
