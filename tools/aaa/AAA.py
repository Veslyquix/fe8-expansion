...
POP"""
  ...
  # --- Raw sidecar dump (scripts/banim_event_to_source.py) -----------------
  # This file's own .event text serialization for framedata has two
  # undocumented quirks that make it fragile to reverse-parse: a fake
  # LZ77-style header (int_to_EA((size<<8)|0x10, 4) + a literal padding
  # byte) even though nothing here is actually compressed, and a stray
  # extra "0" byte token emitted on every other 4-byte word regardless of
  # pointer status (the "addZero" toggle below). Dump the already-correct
  # in-memory frameData buffer directly instead, plus the exact byte offset
  # and sheet index of each pointer word -- computed with the identical
  # isPointer test the text serializer below uses, so it's provably
  # consistent with it. Purely additive: does not change any existing
  # output or behaviour.
  with open(animName + "_framedata.raw.bin", "wb") as _raw:
    _raw.write(frameData)
  with open(animName + "_framedata.pointers.txt", "w") as _ptrs:
    _isPointer = False
    for _i in range(0, len(frameData), 4):
      if _isPointer:
        _sheetNum = int.from_bytes(frameData[_i:_i+4], byteorder='little')
        _ptrs.write(str(_i) + " " + str(_sheetNum) + "\n")
        _isPointer = False
      elif frameData[_i+3] == 0x86:
        _isPointer = True
  # ---------------------------------------------------------------------
