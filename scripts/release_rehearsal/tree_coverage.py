...
def generate_exclusions_document(repo_root: Path, target_sha: str) -> Dict:
    ...
    return {
        "_comment": (
            "Explicit export exclusions (issue #9 mandatory correction #2; R1/R2/closing-round "
            "fix: this comment is now kind-specific and truthful -- it no longer claims a single "
            "uniform, cross-checked OID shape that was never actually true for both kinds). Every "
            "entry here is a tree member intentionally EXCLUDED from the source release archive, "
            "recorded as exactly one of two kinds with genuinely different semantics: (1) "
            "'gitlink' (e.g. mgfembp) -- exact path, immutable mode '160000', and a mandatory, "
            "strictly cross-checked 40-lowercase-hex 'oid' (its pinned commit, verified against "
            "the live tree by check_partition()/manifest.py's check_gitlink_pins()); a changed, "
            "stale, or missing 'oid' here is a hard failure. (2) 'self_referential_evidence' "
            "(curated in tree_coverage.py's SELF_REFERENTIAL_EVIDENCE_PATHS -- today exactly "
            "docs/release_data/provenance/code.json) -- exact path, mode, and a factual 'reason' "
            "only; its 'oid' is always JSON null, is never populated from the live tree, and is "
            "never claimed or cross-checked as a content-identity fact for this kind (a file "
            "cannot record an immutable hash of its own not-yet-finalized content -- see this "
            "module's own docstring). Disjoint union with docs/release_data/source_allowlist.json's "
            "'paths' MUST equal the complete tree at 'generation_basis_sha' -- "
            "scripts/release_rehearsal/tree_coverage.py's check_partition() mechanically enforces "
            "this (a new tracked path in neither set fails coverage; a 'gitlink' entry whose "
            "mode/oid no longer matches the live tree, or a 'self_referential_evidence' entry "
            "whose path/mode is no longer an exact curated match, is a stale-exclusion failure). "
            "'generation_basis_sha' records which commit this file was last regenerated "
            "against -- it never drives (and is never itself used to compute) the actual "
            "partition/coverage result above; every check instead always re-derives its own "
            "live target_sha -- HEAD, an explicit override, or the staged index -- "
            "independently. It IS, however, mechanically checked for truthfulness (issue #9 "
            "final-review remediation): manifest.check_tree_coverage() always calls "
            "git_source.check_generation_basis_is_commit(), which fails closed if this value "
            "is not itself a real, still-reachable commit object -- e.g. a tree written by a "
            "development-time '--target-sha index' preview (see git_source.write_index_tree) "
            "that was never actually committed, or any other non-commit/dangling object -- so "
            "this field can never silently carry a false or ephemeral commit claim, even "
            "though it is not itself a validated *binding* driving any other result computed "
            "above. Generated by "
...
def main(argv=None) -> int:
    ...
    if args.command == "generate-exclusions":
        if args.write and args.target_sha == "index":
            # issue #9 final-review remediation: mirrors
            # allowlist.py's identical '--write'-from-'index' refusal --
            # see that module's main() for the full rationale. This
            # document's own 'generation_basis_sha' makes the exact same
            # "documents which commit" promise.
            print(
                "error: refusing to --write an 'index'-derived (tree, not commit) "
                "generation basis into the checked-in export-exclusions document -- "
                "commit first, then re-run with '--target-sha HEAD --write' (see "
                "docs/release_process.md); '--target-sha index' without '--write' remains "
                "available for a local, uncommitted preview/verification",
                file=sys.stderr,
            )
            return 2
        try:
