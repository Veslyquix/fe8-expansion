...
#   gba_bios_path path to a legally obtained local gba_bios.bin
#                 (sha1 300c20df...).
set -euo pipefail
...
if [ -z "$BIOS" ]; then
  echo "usage: $0 [version] [work_dir] <gba_bios_path>" >&2
  echo "get_gbahawk: a legally obtained local GBA BIOS is required" >&2
  exit 2
fi
...
# The movie requires the real GBA BIOS (sha1 300c20df...); verify the local copy.
fw="$(dirname "$exe")/Firmware/gba_bios.bin"
got="$(sha1sum "$BIOS" | cut -d' ' -f1)"
[ "$got" = "$BIOS_SHA1" ] || {
  echo "GBA BIOS sha1 $got != $BIOS_SHA1 (need the real BIOS)" >&2
  exit 1
}
cp "$BIOS" "$fw"
